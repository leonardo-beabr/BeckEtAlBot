        ...
        return new Promise( function(resolve, reject){
            weather.find({search: '88075-120, SC', degreeType: 'C'}, function(err, result) {
                if(err){console.log(err);reject(err)}
                    ...
                    const phrase = `A temperatura atual em ${result[0]['location']['name']} é de ${result[0]['current']['temperature']}°C com máxima de ${result[0]['forecast'][1]['high']}°C e mínima de ${result[0]['forecast'][1]['low']}°C.`
                    const weatherInfo = {
